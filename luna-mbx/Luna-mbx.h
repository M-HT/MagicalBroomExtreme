...
public:
	...
	static int Drive2( int argc, char *argv[] );
	...
	static void MsgBox( const char *title, const char *string,... );
...
public:
	...
	static int GetBBFormat( void );
	//static LPDIRECT3DSURFACE8 GetBackBuffer( void );
	static unsigned int GetBackBuffer( void );
...
public:
	...
	virtual const char *LoadLAG( const char *file, const char *name );
	...
	virtual BOOL CopyToSurface( void *dest, long pitch, int fmt );
};
...
public:
	...
	static LTEXTURE LoadLAG( const char *pack, const char *data, BOOL alpha = false );
	...
	virtual const char *Refresh( void );
	...
	virtual unsigned int GetTexture( void );
...
public:
	...
	static BOOL SaveData( const char *file, void *pData, unsigned long size );
	//static BOOL LoadData( char *file, void *pData, unsigned long size );
	static BOOL LoadData( const char *file, void *pData, unsigned long size );
